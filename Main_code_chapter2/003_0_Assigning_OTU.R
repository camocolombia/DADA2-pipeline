...
# taxa_silva <- dada2::assignTaxonomy(seqs=colnames(seqtab.nochim)[1],#seqtab.nochim,
...
#                              tryRC = T)
  ...
  taxa_silva <-  lapply(1:ncol(seqtab.nochim),function(i){
    cat(round((i/ncol(seqtab.nochim)*100),2)," % SILVA","\n")
    x <- dada2::assignTaxonomy(seqs=colnames(seqtab.nochim)[i],#seqtab.nochim,
                                 refFasta=paste0(taxadb_dir,"/","silva_nr_v132_train_set.fa.gz"),
                                 multithread = F,
                                 verbose = F,
                                 minBoot=80,
  ...
  taxa_silva <- do.call(rbind,taxa_silva)
...
#                            tryRC = T)
  ...
  taxa_rdp <-  lapply(1:ncol(seqtab.nochim),function(i){
    cat(round((i/ncol(seqtab.nochim)*100),2)," % RDP","\n")
    x <- dada2::assignTaxonomy(seqs=colnames(seqtab.nochim)[i],#seqtab.nochim,
                               refFasta=paste0(taxadb_dir,"/","rdp_train_set_16.fa.gz"),
                               multithread = F,
                               verbose = F,
  ...
  taxa_rdp <- do.call(rbind,taxa_rdp)
...
taxa_rdp  <- matrix(ncol =ncol(taxa_silva),nrow=nrow(taxa_silva))
colnames(taxa_rdp) <- colnames(taxa_silva);rownames(taxa_rdp) <- rownames(taxa_silva)
genus.species_rdp <- matrix(ncol =ncol(genus.species_silva),nrow=nrow(genus.species_silva))
colnames(genus.species_rdp) <- colnames(genus.species_silva);rownames(genus.species_rdp) <- rownames(genus.species_silva)
...
#                           tryRC = T)
  ...
  taxa_gg <-  lapply(1:ncol(seqtab.nochim),function(i){
    cat(round((i/ncol(seqtab.nochim)*100),2)," % GG","\n")
    x <- dada2::assignTaxonomy(seqs=colnames(seqtab.nochim)[i],#seqtab.nochim,
                               refFasta=paste0(taxadb_dir,"/","gg_13_8_train_set_97.fa.gz"),
                               multithread = F,
                               verbose = F,
...
taxa_gg <- readRDS(paste0(seq_dir,"/","taxa_gg.RDS"))
  ...
  taxa_gg  <- matrix(ncol =ncol(genus.species_silva),nrow=nrow(genus.species_silva))
  colnames(taxa_gg) <- colnames(genus.species_silva);rownames(taxa_gg) <- rownames(genus.species_silva)
...
if(!file.exists(paste0(seq_dir,"/","taxa_hungate.RDS"))){
  ...
  taxa_hungate <-  lapply(1:ncol(seqtab.nochim),function(i){
    cat(round((i/ncol(seqtab.nochim)*100),2)," % HUNGATE","\n")
    x <- dada2::assignTaxonomy(seqs=colnames(seqtab.nochim)[i],#seqtab.nochim,
                               refFasta=paste0(taxadb_dir,"/","all_rumen_16S_RG_TrainSet.fasta"),
                               multithread = F,
                               verbose = F,
  ...
  taxa_hungate <- do.call(rbind,taxa_hungate)
