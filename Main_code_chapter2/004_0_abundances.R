...
s1bp <- ggplot(data=sample_data(ps),aes(x=Phase,y=Shannon,fill=Breed))+
  geom_boxplot(position = "dodge",outlier.colour = NA,
               outlier.fill=NA,outlier.alpha=1,
               outlier.size =NA,na.rm = T,show.legend = T) +
  ...
  ggtitle("")+
  scale_fill_manual("Groups", values = c("blue","red","green"))+
...
###Treatment
s1bp_T <- ggplot(data=sample_data(ps),aes(x=Breed,y=Shannon,fill=Breed))+
  geom_boxplot(position = "dodge",outlier.colour = NA,
               outlier.fill=NA,outlier.alpha=1,
               outlier.size =NA,na.rm = T,show.legend = T) +
  stat_boxplot(geom ='errorbar') +
...
s1bp_SI <- ggplot(data=sample_data(ps),aes(x=Phase,y=Simpson,fill=Breed))+
  geom_boxplot(position = "dodge",outlier.colour = NA,
               outlier.fill=NA,outlier.alpha=1,
               outlier.size =NA,na.rm = T,show.legend = T) +
  stat_boxplot(geom ='errorbar') +
...
####
s1bp_SI_T <- ggplot(data=sample_data(ps),aes(x=Breed,y=Simpson,fill=Breed))+
  geom_boxplot(position = "dodge",outlier.colour = NA,
               outlier.fill=NA,outlier.alpha=1,
               outlier.size =NA,na.rm = T,show.legend = T) +
  stat_boxplot(geom ='errorbar') +
...
require(ggpubr)
sample_data(ps)$groups <- paste0(sample_data(ps)$Breed)#," (",sample_data(ps)$Phase,")")
...
sp <- ggpubr::ggscatterhist(
  data= sample_data(ps), x = "Shannon", y = "ADG",
...
Cheviot_SIMPSON = describe(subset(sample_data(ps),Breed=="Cheviot")$Simpson),
Connemara_SIMPSON  = describe(subset(sample_data(ps),Breed=="Connemara")$Simpson),
Lanark_SIMPSON  = describe(subset(sample_data(ps),Breed=="Lanark")$Simpson),
Cheviot_SHANNON = describe(subset(sample_data(ps),Breed=="Cheviot")$Shannon),
Connemara_SHANNON= describe(subset(sample_data(ps),Breed=="Connemara")$Shannon),
Lanark_SHANNON = describe(subset(sample_data(ps),Breed=="Lanark")$Shannon)
)

descriptive_table <- do.call(rbind, descriptive_table)
descriptive_table <- as.data.frame(descriptive_table)
descriptive_table$trait <- NA; descriptive_table$trait <- row.names(descriptive_table)
descriptive_table <- descriptive_table[,c(14,1:13)]

write.csv(descriptive_table,paste0(out_dir,"/","csv/","describe_alpha.csv"),quote = F,row.names = F)

w <-agricolae::kruskal(sample_data(ps)$Simpson, sample_data(ps)$Breed, alpha = 0.05,group=F)
w2 <-agricolae::kruskal(sample_data(ps)$Shannon, sample_data(ps)$Breed, alpha = 0.05,group=F)
        #                    type=2)$mean,
wilcox_list_DIV <- data.frame(
  Simpson_Chisq  = w$statistics[1],
  Simpson_df = w$statistics[2],
  Simpson_pvalue  = w$statistics[3],
  
  Shannon_Chisq  = w2$statistics[1],
  Shannon_df  = w2$statistics[2],
  Shannon_pvalue  = w2$statistics[3]
#   ###########################################################################
#   #FEED EFFICIENCY SIMPSON
# SIMP_HE = describe(subset(sample_data(ps),ADG_Treatment=="High feed efficiency")$Simpson,
#                    type=2)$mean,
# SIMP_HE_SE = describe(subset(sample_data(ps),ADG_Treatment=="High feed efficiency")$Simpson,
#                    type=2)$sd,
# SIMP_LE = describe(subset(sample_data(ps),ADG_Treatment=="Low feed efficiency")$Simpson,
#                    type=2)$mean,
# SIMP_LE_SE = describe(subset(sample_data(ps),ADG_Treatment=="Low feed efficiency")$Simpson,
#                    type=2)$sd,
# SIMP_EFF_pvalue = wilcox.test(subset(sample_data(ps),ADG_Treatment=="High feed efficiency")$Simpson,
#                        subset(sample_data(ps),ADG_Treatment=="Low feed efficiency")$Simpson)$p.value,
# ###########################################################################
# #FEED EFFICIENCY SHANNON
# SHANN_HE = describe(subset(sample_data(ps),ADG_Treatment=="High feed efficiency")$Shannon,
#                    type=2)$mean,
# SHANN_HE_SD = describe(subset(sample_data(ps),ADG_Treatment=="High feed efficiency")$Shannon,
#                       type=2)$sd,
# SHANN_LE = describe(subset(sample_data(ps),ADG_Treatment=="Low feed efficiency")$Shannon,
#                    type=2)$mean,
# SHANN_LE_SD = describe(subset(sample_data(ps),ADG_Treatment=="Low feed efficiency")$Shannon,
#                       type=2)$sd,
# SHANN_EFF_pvalue = wilcox.test(subset(sample_data(ps),ADG_Treatment=="High feed efficiency")$Shannon,
#                        subset(sample_data(ps),ADG_Treatment=="Low feed efficiency")$Shannon)$p.value,
# ###########################################################################
# #PHASE SIMPSON
...
# ###########################################################################
# #PHASE SIMPSON HFE
# SIMP_SO_HE = describe(subset(sample_data(ps),Phase=="Solid" & ADG_Treatment=="High feed efficiency")$Simpson,
#                    type=2)$mean,
# SIMP_SO_HE_SD = describe(subset(sample_data(ps),Phase=="Solid" & ADG_Treatment=="High feed efficiency")$Simpson,
#                       type=2)$sd,
# SIMP_LI_HE = describe(subset(sample_data(ps),Phase=="Liquid"& ADG_Treatment=="High feed efficiency")$Simpson,
#                    type=2)$mean,
# SIMP_LI_HE_SD = describe(subset(sample_data(ps),Phase=="Liquid" & ADG_Treatment=="High feed efficiency")$Simpson,
#                       type=2)$sd,
# SIMP_PHASE_HE_pvalue = wilcox.test(subset(sample_data(ps),Phase=="Liquid"  & ADG_Treatment=="High feed efficiency")$Simpson,
#                                 subset(sample_data(ps),Phase=="Solid" & ADG_Treatment=="High feed efficiency")$Simpson)$p.value,
# ###########################################################################
# #PHASE SHANNON HFE
# SHANN_O_HE = describe(subset(sample_data(ps),Phase=="Solid" & ADG_Treatment=="High feed efficiency")$Shannon,
#                       type=2)$mean,
# SHANN_SO_HE_SD = describe(subset(sample_data(ps),Phase=="Solid" & ADG_Treatment=="High feed efficiency")$Shannon,
#                          type=2)$sd,
# SHANN_LI_HE = describe(subset(sample_data(ps),Phase=="Liquid"& ADG_Treatment=="High feed efficiency")$Shannon,
#                       type=2)$mean,
# SHANN_LI_HE_SD = describe(subset(sample_data(ps),Phase=="Liquid" & ADG_Treatment=="High feed efficiency")$Shannon,
#                          type=2)$sd,
# SHANN_PHASE_HE_pvalue = wilcox.test(subset(sample_data(ps),Phase=="liquid"  & ADG_Treatment=="High feed efficiency")$Shannon,
#                                    subset(sample_data(ps),Phase=="solid" & ADG_Treatment=="High feed efficiency")$Shannon)$p.value,
# ###########################################################################
# #PHASE SIMPSON LFE
# SIMP_SO_LE = describe(subset(sample_data(ps),Phase=="Solid" & ADG_Treatment=="Low feed efficiency")$Simpson,
#                       type=2)$mean,
# SIMP_SO_LE_SD = describe(subset(sample_data(ps),Phase=="Solid" & ADG_Treatment=="Low feed efficiency")$Simpson,
#                          type=2)$sd,
# SIMP_LI_LE = describe(subset(sample_data(ps),Phase=="Liquid"& ADG_Treatment=="Low feed efficiency")$Simpson,
#                       type=2)$mean,
# SIMP_LI_LE_SD = describe(subset(sample_data(ps),Phase=="liquid" & ADG_Treatment=="Low feed efficiency")$Simpson,
#                          type=2)$sd,
# SIMP_PHASE_LE_pvalue = wilcox.test(subset(sample_data(ps),Phase=="liquid"  & ADG_Treatment=="Low feed efficiency")$Simpson,
#                                    subset(sample_data(ps),Phase=="solid" & ADG_Treatment=="Low feed efficiency")$Simpson)$p.value,
# ##########################################################################
# #PHASE SHANNON LFE
# SHANN_SO_LE = describe(subset(sample_data(ps),Phase=="Solid" & ADG_Treatment=="Low feed efficiency")$Shannon,
#                       type=2)$mean,
# SHANN_SO_LE_SD = describe(subset(sample_data(ps),Phase=="Solid" & ADG_Treatment=="Low feed efficiency")$Shannon,
#                          type=2)$sd,
# SHANN_LI_LE = describe(subset(sample_data(ps),Phase=="Liquid"& ADG_Treatment=="Low feed efficiency")$Shannon,
#                       type=2)$mean,
# SHANN_LI_LE_SD = describe(subset(sample_data(ps),Phase=="Liquid" & ADG_Treatment=="Low feed efficiency")$Shannon,
#                          type=2)$sd,
# SHANN_PHASE_LE_pvalue = wilcox.test(subset(sample_data(ps),Phase=="Liquid"  & ADG_Treatment=="Low feed efficiency")$Shannon,
#                                    subset(sample_data(ps),Phase=="Solid" & ADG_Treatment=="Low feed efficiency")$Shannon)$p.value,
# ###########################################################################
# #COMPARING LIQUID FRACTIONS
# SIMP_LIQs_pvalue = wilcox.test(subset(sample_data(ps),Phase=="Liquid"  & ADG_Treatment=="Low feed efficiency")$Simpson,
#                                    subset(sample_data(ps),Phase=="Liquid" & ADG_Treatment=="High feed efficiency")$Simpson)$p.value,
# #####
# SHANN_LIQs_pvalue = wilcox.test(subset(sample_data(ps),Phase=="Liquid"  & ADG_Treatment=="Low feed efficiency")$Shannon,
#                                subset(sample_data(ps),Phase=="Liquid" & ADG_Treatment=="High feed efficiency")$Shannon)$p.value,
# 
# ###########################################################################
# #COMPARING SOLID FRACTIONS
# SIMP_SOLs_pvalue = wilcox.test(subset(sample_data(ps),Phase=="Solid"  & ADG_Treatment=="Low feed efficiency")$Simpson,
#                                subset(sample_data(ps),Phase=="Solid" & ADG_Treatment=="High feed efficiency")$Simpson)$p.value,
# #####
# SHANN_SOLs_pvalue = wilcox.test(subset(sample_data(ps),Phase=="Solid"  & ADG_Treatment=="Low feed efficiency")$Shannon,
#                                 subset(sample_data(ps),Phase=="Solid" & ADG_Treatment=="High feed efficiency")$Shannon)$p.value,
# 
# ###########################################################################
# ###########################################################################
