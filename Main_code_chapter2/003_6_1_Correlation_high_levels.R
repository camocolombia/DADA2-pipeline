...
# HFE_Sub <- phyloseq::subset_samples(ps_object2[[i]],ADG_Treatment="High feed efficiency")
# LFE_Sub <- phyloseq::subset_samples(ps_object2[[i]],ADG_Treatment="Low feed efficiency")
# LI_Sub <- phyloseq::subset_samples(ps_object2[[i]],Phase="liquid")
# SO_Sub <- phyloseq::subset_samples(ps_object2[[i]],Phase="solid")


   HFE_Sub <- as.data.frame((otu_table(subset_samples(ps_object2[[i]], ADG_Treatment=="High feed efficiency"))))
   LFE_Sub <- as.data.frame((otu_table(subset_samples(ps_object2[[i]], ADG_Treatment=="Low feed efficiency"))))
  ...
  B1_Sub <- as.data.frame((otu_table(subset_samples(ps_object2[[i]], Breed=="Connemara"))))
  B2_Sub <- as.data.frame((otu_table(subset_samples(ps_object2[[i]], Breed=="Cheviot"))))
  B3_Sub <- as.data.frame((otu_table(subset_samples(ps_object2[[i]], Breed=="Lanark"))))
  
  
correlation.table <- microbiome::associate(ps_object2[[i]]@otu_table,meta(ps3)[,-c(1:5,8,13,15)][,c(2,5:7)],# meta(ps_object2[[i]])[,-c(1:4,10)], 
                                           method = "spearman",
...
tax_table_i$B1_mean <- colMeans(B1_Sub)
tax_table_i$B1_sd <- matrixStats::colSds(as.matrix(B1_Sub))
tax_table_i$B2_mean <- colMeans(B2_Sub)
tax_table_i$B2_sd <- matrixStats::colSds(as.matrix(B2_Sub))
tax_table_i$B3_mean <- colMeans(B3_Sub)
tax_table_i$B3_sd <- matrixStats::colSds(as.matrix(B3_Sub))
...
correlation.table_taxa <- correlation.table_taxa[order(correlation.table_taxa$Variable,correlation.table_taxa$Correlation),]
correlation.table_taxa <- correlation.table_taxa[,c(1,24,5,2:4,24,6:23)]
correlation.table_taxa2 <- correlation.table_taxa[which(correlation.table_taxa$p.adj <0.05),]
write.csv(correlation.table_taxa,paste0(out_dir,"/","csv/","correlation.table_high_taxomomy_full.csv"),quote = F,row.names = F)
write.csv(correlation.table_taxa2,paste0(out_dir,"/","csv/","correlation.table_high_taxomomy_full_filtered.csv"),quote = F,row.names = F)
