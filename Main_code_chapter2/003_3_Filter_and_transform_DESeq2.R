...
HL <-as.data.frame(t(otu_table(subset_samples(ps, ADG_Treatment=="High feed efficiency" & Phase=="Liquid"))))
HS <-as.data.frame(t(otu_table(subset_samples(ps, ADG_Treatment=="High feed efficiency" & Phase=="Solid"))))
HE <-as.data.frame(t(otu_table(subset_samples(ps, ADG_Treatment=="High feed efficiency" & Phase=="Epithelial"))))

LL <-as.data.frame(t(otu_table(subset_samples(ps, ADG_Treatment=="Low feed efficiency" & Phase=="Liquid"))))
LS <-as.data.frame(t(otu_table(subset_samples(ps, ADG_Treatment=="Low feed efficiency" & Phase=="Solid"))))
LE <-as.data.frame(t(otu_table(subset_samples(ps, ADG_Treatment=="Low feed efficiency" & Phase=="Epithelial"))))

##Summarizing libraries size


lib_df <- data.frame(samples = c(names(HL),names(HS),names(HE),names(LL),names(LS),names(LE)),
                        sampleType = c(rep("liquid",length(HL)),
                                       rep("solid",length(HS)),
                                       rep("epithelial",length(HE)),
                                       rep("liquid",length(LL)),
                                       rep("solid",length(LS)),
                                      rep("epithelial",length(LE))
                     ),
                        Treatment = c(rep("High feed efficiency",length(HL)),
                                      rep("High feed efficiency",length(HS)),
                                      rep("High feed efficiency",length(HE)),
                                      rep("Low feed efficiency",length(LL)),
                                      rep("Low feed efficiency",length(LS)), 
                                      rep("Low feed efficiency",length(LE))),
                          libSize = as.numeric(c(colSums(HL),colSums(HS),colSums(HE),colSums(LL),colSums(LS),colSums(LE)))
                        )
...
ps <- subset_samples(ps, Phase %in% c("Liquid","Solid"))
ps <- subset_taxa(ps, Phylum!=is.na(Phylum))
ps <- subset_samples(ps, Subject %!in% outliers$Outlier)
prod_used <- as(ps@sam_data,"data.frame")
prod_used <- prod_used[!duplicated(prod_used$sampleID),]
table(prod_used$Breed)
...
lib_df <- lib_df[which(lib_df$samples %in% c(as.character(colnames(HL)),
                                             as.character(colnames(HS)),
                                             #as.character(colnames(HE)),
                                             as.character(colnames(LL)),
                                             as.character(colnames(LS))
                                            # as.character(colnames(LE))
)
),]
lib_df$libSize_T <- NA;lib_df$libSize_T <- as.numeric(c(colSums(HL),colSums(HS),colSums(LL),#colSums(HE),
                                                        colSums(LS)))#,#colSums(LE)))
...
ps_tree2 <- plot_tree(ps,nodelabf=nodeplotboot(),# nodelabf=nodeplotboot(80,0,3),
  ...
  scale_color_manual("Treatment", values = c("blue","red","green"))+
